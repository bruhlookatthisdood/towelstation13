...
/obj/machinery/mineral/ore_redemption/RefreshParts()
	...
	for(var/datum/stock_part/matter_bin/B in component_parts)
		ore_multiplier_temp = ore_multiplier_temp * ((0.50 * B.tier) + (1 - 0.50))
		to_chat(world, "B rating: [B.tier]")
	for(var/datum/stock_part/micro_laser/L in component_parts)
		point_upgrade_temp = point_upgrade_temp * ((0.50 * L.tier) + (1 - 0.50))
		to_chat(world, "L rating: [L.tier]")
